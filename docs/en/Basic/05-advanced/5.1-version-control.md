...
This chapter assumes that your digital twin's chat area can already call a model. If the page still returns placeholder responses, or you have not configured `AI_API_KEY`, `AI_BASE_URL`, and `AI_MODEL`, start here:
- [4.7 API Integration in Practice: Configure a Model API and Run a Minimal Connection Test](/Advanced/04-dev-fundamentals/07-api-integration)

To continue learning about the request path from the page to the backend, API design, and frontend-backend collaboration, read:
